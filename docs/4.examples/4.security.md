    ...
    for (const { regex, name } of SECRET_PATTERNS) {
      if (regex.test(context.content)) {
        errors.push(
          ...
          `   ℹ️  Example: process.env.API_KEY\n` +
  ...
  validate(context) {
    ...
    if (/v-html\s*=/.test(context.content)) {
      errors.push(
        ...
        `   ℹ️  Consider using a library like DOMPurify\n` +
        `   📄 File: ${context.filePath}`
      );
    }
    ...
    for (const pattern of sqlConcatPatterns) {
      if (pattern.test(context.content)) {
        errors.push(
          ...
          `   ℹ️  Example: db.query('SELECT * FROM users WHERE id = ?', [userId])\n` +
          `   📄 File: ${context.filePath}`
        );
        break;
      }
    ...
    if (/Math\.random\(\)/.test(context.content)) {
      ...
      if (isSensitive) {
        errors.push(
          ...
          `   ℹ️  Example: crypto.randomBytes(32).toString('hex')\n` +
          `   📄 File: ${context.filePath}`
        );
      }
    }
